...
const int _androidInlinePayloadLimitBytes = 256 * 1024;
...
class PlatformAdaptiveTransport implements AppTransport {
  ...
  PlatformAdaptiveTransport({
    ...
    _relayTransport.messages.listen(_messages.add, onError: _messages.addError);
  }
...
class AndroidForegroundTransport implements AppTransport {
  ...
  Future<void> send(String payload) async {
    ...
    await _sendAndroidPayload(_methods, payload);
  }
...
class AndroidRelaySecureTransport implements AppTransport {
  ...
  Future<void> connect(AppSettings settings) async {
    ...
    if (!_isAllowedRelayUri(relayUri)) {
      throw StateError(
        'Relay mode requires HTTPS for non-local relay servers.',
      );
    }
    _settings = settings;
    final readyCompleter = Completer<void>();
  ...
  Future<void> _completeSession(Map<String, dynamic> payload) async {
    ...
    if (peerSigningKey != settings.relayBridgeSigningPublicKey) {
      throw StateError(
        'Relay bridge identity does not match the paired bridge.',
      );
    }
...
Future<void> _sendAndroidPayload(MethodChannel methods, String payload) async {
  final payloadBytes = utf8.encode(payload);
  if (payloadBytes.length <= _androidInlinePayloadLimitBytes) {
    await methods.invokeMethod<void>('send', <String, dynamic>{
      'payload': payload,
    });
    return;
  }

  final file = await _writeAndroidTransportPayload(payload);
  await methods.invokeMethod<void>('sendFile', <String, dynamic>{
    'path': file.path,
  });
}

Future<File> _writeAndroidTransportPayload(String payload) async {
  final directory = await Directory(
    '${Directory.systemTemp.path}${Platform.pathSeparator}codex_remote_payloads',
  ).create(recursive: true);
  final file = File(
    '${directory.path}${Platform.pathSeparator}'
    'payload-${DateTime.now().microsecondsSinceEpoch}-${Random().nextInt(1 << 32)}.json',
  );
...
Uri relayWebSocketUri(Uri relayUri) {
  ...
  final normalizedPath = relayUri.path.endsWith('/')
      ? '${relayUri.path}ws'
      : '${relayUri.path}/ws';
  return relayUri.replace(scheme: wsScheme, path: normalizedPath);
}
...
Object _sortJson(Object value) {
  ...
  if (value is List<dynamic>) {
    return value.map((dynamic item) => _sortJson(item)).toList(growable: false);
  }
  return value;
}
